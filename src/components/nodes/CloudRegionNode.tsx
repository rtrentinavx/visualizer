...
export default memo(function CloudRegionNode({ data }: NodeProps<CloudRegionNode>) {
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-elevated)] px-4 py-3 min-w-[180px]">
      <Handle type="target" position={Position.Top} className="!w-2 !h-2 !bg-[var(--color-accent-blue)] !border-white" />
      ...
      <Handle type="source" position={Position.Bottom} className="!w-2 !h-2 !bg-[var(--color-accent-blue)] !border-white" />
    </div>
  );
});
